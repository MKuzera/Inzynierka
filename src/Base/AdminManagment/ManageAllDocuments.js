...
import './ManageAllDocuments.css';
import { GetDocuments, UpdateDocument, DeleteDocument } from '../ApiServices/DocumentService';
...
const ManageAllDocuments = ({ setActivePage }) => {
    ...
    const handleInputChange = (e) => {
        ...
        if (name === 'dateAdded') {
            // Ensure the date value is in correct format
    ...
    const handleDelete = async (documentId) => {
        try {
            ...
            const data = await GetDocuments(authState.token);
            const formattedData = data.map((doc) => ({
                ...doc,
                tags: doc.tags.split(',').map((tag) => tag.trim()),
                dateAdded: new Date(doc.dateAdded).toISOString().split('T')[0],
